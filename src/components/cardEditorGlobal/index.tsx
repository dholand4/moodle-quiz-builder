...
export function cardsToText(cards: IQuestionCard[]): string {
  return cards
    .map((card, i) => {
      ...
      const feedbackLine = card.feedback?.trim() ? `feedback: ${card.feedback.trim()}` : null;
      return [header, ...opts, ...(feedbackLine ? [feedbackLine] : [])].join('\n');
    })
    .join('\n\n');
}
        ...
        {cards.map((card, cardIndex) => {
          ...
          return (
            <S.Card key={card.id} $hasError={hasError} $isOk={isOk}>
              ...
              <S.FeedbackDivider />
              <S.FeedbackSection>
                <S.FeedbackLabel>Feedback geral</S.FeedbackLabel>
                <AutoGrow
                  className="feedback-area"
                  value={card.feedback ?? ''}
                  placeholder="Explicação exibida após o aluno responder (opcional)…"
                  onChange={(v) => updateCard(card.id, { feedback: v })}
                />
              </S.FeedbackSection>
            </S.Card>
          );
